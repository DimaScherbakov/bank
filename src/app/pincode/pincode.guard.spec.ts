...
describe('PincodeGuard', () => {
  ...
  it('should pass the route when card with correct data exist (currentCard)', inject([PincodeGuard], (guard: PincodeGuard) => {
    const guard = TestBed.get(PincodeGuard);
    guard.pincodeService.findCurrentCard = () => { guard.pincodeService.currentCard = {}};
    expect(guard.canActivate()).toEqual(true);
  }));
  it('should not pass the route when card is undefined (currentCard)', inject([PincodeGuard], (guard: PincodeGuard) => {
    const guard = TestBed.get(PincodeGuard);
